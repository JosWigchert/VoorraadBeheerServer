namespace VoorraadBeheer
{
    ...
    {
        public IEnumerable<T> Store { get; set; }
        public Func<T, string> GetString { get; set; }

        public void Bind(IEnumerable<T> toSort)
        {
            Store = toSort;
        }

        public IEnumerable<T> CheckItems(string toCheck)
        {
            if (GetString == null) 
                return Store;
            return Store.OrderBy(x => DamerauLevenshteinDistance(toCheck, GetString(x), 100));
        }
